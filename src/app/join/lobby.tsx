...
const PROMPTS = [
  ...
  { emoji: "✈️", text: "Find someone who has visited the same country as you" },
...
export default function LobbyScreen() {
  ...
  const checkPassphrase = () => {
    ...
    setFeedback({
      type: "success",
      message: `Connected with ${match.emoji} ${match.name}!`,
    });
    setEntered([]);
  };
  ...
  if (activePrompt) {
    const allConnected =
      promptGroup.length > 0 && connectedIds.length === promptGroup.length;
    return (
      <SafeAreaView
      ...
      >
        <View style={styles.promptContent}>
          ...
          <View style={styles.passShareRow}>
            <Text style={styles.passShareLabel}>Your passphrase</Text>
            ...
            <>
              ...
              <View style={styles.enteredRow}>
                {entered.length === 0 ? (
                  ...
                  entered.map((word, i) => (
                    ...
                    >
                      <Text style={styles.enteredChipText}>{word}</Text>
                    </TouchableOpacity>
                  ))
                )}
              </View>
              ...
              <View style={styles.wordbank}>
                {wordbank.map((word) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={word}
                      style={[styles.wordChip, chosen && styles.wordChipChosen]}
                      onPress={() => toggleWord(word)}
                    >
                      <Text
                        style={[
                          styles.wordChipText,
                          chosen && styles.wordChipTextChosen,
                        ]}
                      >
                        {word}
                      </Text>
              ...
              <View style={styles.challengeButtons}>
                <TouchableOpacity
                  ...
                  disabled={entered.length !== PASSPHRASE_LENGTH}
                >
                  <Text style={styles.doneButtonText}>Connect</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.doneButtonOutline}
                  onPress={dismissPrompt}
                >
                  <Text style={styles.doneButtonOutlineText}>Done</Text>
                </TouchableOpacity>
              </View>
            </>
          ) : (
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        {code ? <Text style={styles.codeBadge}>Event {code}</Text> : null}
        <Text style={styles.subtitle}>
          Hang tight — challenges will pop up during the event. Keep an eye out!
        </Text>
      </View>
